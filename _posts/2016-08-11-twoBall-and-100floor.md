...
即$$n=\lceil \sqrt{2f}-0.5 \rceil$$当\\\( f = 100 \\)时有:$$n \ge \sqrt{200}-0.5= 13.64 $$ 所以取 $$ n = 14 $$。限制条件 $$\exists m,x_m>f$$ 的意义是找一个尽量大的 $$x_m $$,而 $$x_i \le x_{i-1}+n-i $$ 所以最大的$$x_{i-1}$$就可以使得$$x_i$$取最大值。因此，可以使用限制条件取等号 $$ x_i = x_{i-1}+ n -i+1 $$ 来获得一个满足条件的序列 $$\{x_i\}$$，可得$$ x_i = i*n-(i-1)$$ 即其中一个满足的序列 $$ \{ x_i \} $$ 是 $$\{ 14,27,39,50,60... \}$$。同样当 \\\( f = 39 \\) 时有: $$n \ge \sqrt{74}-0.5= 8.10 $$ 所以取 $$ n = 9 $$,其中一个满足的序列$$ \{ x_i \} $$是 $$\{ 9,17,24,30,35... \}$$。当然很多时候满足上述限制条件的序列可能不止一个。
...
$$ \begin{equation}
n \le \frac{1}{3}p^{1/3}-5p^{-1/3}
\end{equation} $$

其中 $$p=3^4f+\sqrt{15^3+3^8f^2}$$。上述解不能够继续化简了，并且可以取得等号。
如 $$f=92$$ 时, $$n \ge 8$$, $$f=129$$ 时， $$ n \ge 9$$ 。当 $$f = 100 $$ 时，$$n
\ge 8.23 $$。得出 $$n$$ 之后就可以使用公式 $$ x_i =
\frac{1}{2}((n-i)^2+(n-i))+x_{i-1}+1 $$，求得满足条件的一个序列 $$\{x_i\}$$。当
第一个球碎了之后，问题就变成了上面两个球的问题了,上面已经给出了解。
